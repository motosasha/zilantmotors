...
$(function() {
 ...
 //   	 		}
	...
	if ($('.js-slider').length) {
	     ...
	     //  		$status.html( '<strong>' + (((i)<10) ? "0"+(i) : i) +'</strong>'+ '<small>/' + (((slick.slideCount)<10) ? "0"+(slick.slideCount) : slick.slideCount)  + '</<small>');
	    	...
	    	$('.js-slider').slick({
	      		...
	      		pauseOnHover: true,
	      		lazyLoad: 'ondemand',
	      		prevArrow: '<a href="javascript: void(0);" class="page__link arrow arrow__prev"><span><img class="page__img" src="/local/templates/zilant/img/icon/back-black.svg"></span></a>',
	      		nextArrow: '<a href="javascript: void(0);" class="page__link arrow arrow__next"><span><img class="page__img" src="/local/templates/zilant/img/icon/next-black.svg"></span><svg preserveAspectRatio="none" class="progress" viewBox="0 0 110 110"><circle class="circle-bg" r="50" cx="55" cy="55"/><circle class="circle-go" r="50" cx="55" cy="55"/></svg></a>',
	      		autoplay: true,
	      		autoplaySpeed: 9000,
	      		cssEase: 'ease-in-out',
    			touchThreshold: 100,
    ...
    $('.js-slider__min, .js-slider__max, .js-slider-run__min, .js-slider-run__max').find('input').keypress(function(e) {
    	var key, keyChar;
    	if(!e) var e = window.e;
    ...
    $('.js-slider-run__min').find('input').change(function() {
    ...
    $('.js-slider-run__max').find('input').change(function() {
    	var value1 = $('.js-slider-run__min').find('input').val();
    	var value2 = $('.js-slider-run__max').find('input').val();
    	if(parseInt(value1) > parseInt(value2)) {
    		value2 = value1;
    ...
    $('#calc_ui').slider({		
        ...
        change: function(event, ui) {
			...
			$(".inrub_sum").html(String(avans_rub.toFixed()).replace(/(\d)(?=(\d{3})+([^\d]|$))/g, '$1 '));
			$(".price_res").html(String(plata.toFixed()).replace(/(\d)(?=(\d{3})+([^\d]|$))/g, '$1 '));
			$(".calculate__result--summ").html(String(obsh.toFixed()).replace(/(\d)(?=(\d{3})+([^\d]|$))/g, '$1 '));
 ...
 //      			});
	...
	$("#amount1, #amount, #amount3").change(function(){
			...
			//console.log(obsh);
			$('.avans_total').show();
			$(".calculate__result--summ_avans").html(String(avans_rub.toFixed()).replace(/(\d)(?=(\d{3})+([^\d]|$))/g, '$1 '));
$(".price_res").html(String(plata.toFixed()).replace(/(\d)(?=(\d{3})+([^\d]|$))/g, '$1 '));
$(".calculate__result--summ").html(String(obsh.toFixed()).replace(/(\d)(?=(\d{3})+([^\d]|$))/g, '$1 '));	
   ...
   $("a.liz_anchor").click(function() {
	...
	$('.make_servis').click(function(event){
		...
		$('.popup__form input, .popup__form textarea').click(function(){$(this).parent().removeClass('has-error');});
		$('.send_calc').click(function(event){			
			...
			if(res)
			{
				...
				//parent.yaC(cid);
				$.ajax({
					type: "POST", url: "/include/form_handler_cat.php",
					data: form.serialize(),
					datatype: "html",
					cache: false,
		...
		$('.get_subs').click(function(event){			
			...
			if(res && mail)
			{
				...
				//parent.yaC(cid);
				$.ajax({
					type: "POST", url: "/include/subscribe.php",
					data: form.serialize(),
					datatype: "html",
					cache: false,
					success: function(answer){
